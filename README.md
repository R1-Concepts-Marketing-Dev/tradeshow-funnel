...
> **This repo is public. The registry is not.**
> Show names, audience sizes, contact counts and ad spend live in the private
> repo [tradeshow-funnel-data](https://github.com/R1-Concepts-Marketing-Dev/tradeshow-funnel-data).
> Clone it and point `TSF_DATA_DIR` at it before you do anything — `tsf doctor`
> will tell you if you have not. No contact records go in either repo; those
> live in HubSpot.
...
```bash
npm install
cp .env.example .env    # then fill it in, including TSF_DATA_DIR
node bin/tsf.js doctor  # confirms where the registry is and that HubSpot works
node bin/tsf.js ui
```
...
| --- | --- | --- |
| `AUDIENCES.md` | Generated summary. Start here. **Lives in the data repo.** | No — regenerated by `tsf report` |
...
| `src/hubspot.js` — `isWrite()` | Decides what counts as a write, for test mode. |
